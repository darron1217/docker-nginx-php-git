...
if [[ "$ERRORS" != "1" ]] ; then
 echo display_errors = Off >> /etc/php5/conf.d/php.ini
else
 echo display_errors = On >> /etc/php5/conf.d/php.ini
fi

# Enable PHP short tag or not
if [[ "$SHORT_TAG" != "1" ]] ; then
 echo short_open_tag = Off >> /etc/php5/conf.d/php.ini
else
 echo short_open_tag = On >> /etc/php5/conf.d/php.ini
fi
...
if [ ! -z "$TIMEZONE" ]; then
 ...
 sed -i "s/date.timezone = UTC/date.timezone = ${TIMEZONE}M/g" /etc/php5/conf.d/php.ini
fi
